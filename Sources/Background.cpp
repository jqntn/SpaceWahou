...
const float INITIAL_SPEED =
  500.0f; // Initial speed of entity. Value in pixel per second.
const float FRICTION =
  -0.5f; // Friction that will reduce speed during lifetime.
const float ROTATION_SPEED =
  5.0f; // Entity will spin on self. Value in degree per second.
const float LIFE_TIME = 5.0f; // Entity will live during this duration. After
                              // what, it will be ready to be destroyed.
const float FADE_DURATION = 1.0f; // Duration at end of entity life where entity
                                  // will "fade" (color alpha from 0 to 1)
...
void
Background::Update(sf::RenderWindow* window, float deltaTime)
{
...
// ENTITY BACKGROUND
Entity*
Background::CreateEntity(float x, float y, float dirAngle)
{
  ...
  pEntity->direction = { cos(ConvertDegToRad(dirAngle)),
                         sin(ConvertDegToRad(dirAngle)) };
  return pEntity;
}
...
void
Star::DrawStars(sf::RenderWindow* window)
{
  window->draw(star);
}
void
Background::UpdateB(Entity* pEntity, float deltaTime)
{
...
void
Background::DrawEntity(Entity* pEntity, sf::RenderWindow& window)
{
  window.draw(pEntity->shape);
}
// Check if entity is alive (timeToLive > 0)
bool
Background::IsAlive(Entity* pEntity)
{
  return pEntity->timeToLive > 0;
}
// Deallocate everything that entity possess
void
Background::Destroy(Entity* pEntity)
{
  delete pEntity;
}
void
Background::CreateStars(sf::RenderWindow& window)
{
  // STARS
  std::list<Star*>::iterator starsIt = stars.begin();
  for (int i = 0; i < NUMBER_STARS; i++) {
    float randomX = rand() * (float)window.getSize().x / (float)RAND_MAX;
    float randomY = rand() * (float)window.getSize().y / (float)RAND_MAX;
...
void
Background::SpawnEntities(sf::RenderWindow& window, float deltaTime)
{
...
void
Background::EndEntities(float deltaTime)
{
  std::list<Entity*>::iterator entityIt = entities.begin();
  while (entityIt != entities.end()) {
