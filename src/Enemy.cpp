...
Enemy::Enemy(std::string id, sf::Vector2f position, Type type)
{
	...
	case Type::Triangle:
	{
		...
		{
			soundBuffers[Type::Triangle].loadFromFile(getAssetsPath() + "\\monster0.ogg");
		}
		// isSoundLoaded = buffer_death.loadFromFile(getAssetsPath() + "\\monster0.ogg");
	}
	break;
	case Type::Square:
	{
		...
		{
			soundBuffers[Type::Square].loadFromFile(getAssetsPath() + "\\monster1.ogg");
		}
		// isSoundLoaded = buffer_death.loadFromFile(getAssetsPath() + "\\monster1.ogg");
	}
	break;
	case Type::Circle:
	{
		...
		{
			soundBuffers[Type::Circle].loadFromFile(getAssetsPath() + "\\monster2.ogg");
		}
		// isSoundLoaded = buffer_death.loadFromFile(getAssetsPath() + "\\monster2.ogg");
	}
	break;
	case Type::Octagon:
	{
		...
		{
			soundBuffers[Type::Octagon].loadFromFile(getAssetsPath() + "\\monster3.ogg");
		}
		// isSoundLoaded = buffer_death.loadFromFile(getAssetsPath() + "\\monster3.ogg");
	}
	break;
	}
...
void Enemy::Update(sf::RenderWindow *window, float deltaTime)
{
	...
	if (health <= 0)
	{
		...
		tmp->play();
		Map::mape->compteurEnemy--;
		ParticleSystem *particles = new ParticleSystem(150, 1.2f, this->shape.getPosition(), color, {3, 3});
		player->score += score;
		if (loot <= 75)
